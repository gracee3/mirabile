...
impl WorkerCalculationRuntime {
    pub fn deterministic() -> Self {
        let descriptor = mirabile_engine::CalculationBackend::descriptor(&DeterministicBackend);
        match create_worker() {
    ...
    #[cfg(feature = "xalen-backend")]
    pub fn xalen() -> Self {
        let descriptor = mirabile_engine::CalculationBackend::descriptor(&XalenBackend);
        match create_worker() {
            Ok(worker) => Self::with_worker(descriptor, worker),
            Err(error) => Self {
                inner: Rc::new(WorkerRuntimeInner {
